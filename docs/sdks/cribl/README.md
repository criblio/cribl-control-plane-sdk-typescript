...
async function run() {
  const result = await criblControlPlane.system.settings.cribl.get();
...
import { systemSettingsCriblGet } from "cribl-control-plane/funcs/systemSettingsCriblGet.js";
...
async function run() {
  const res = await systemSettingsCriblGet(criblControlPlane);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("systemSettingsCriblGet failed:", res.error);
  }
}
