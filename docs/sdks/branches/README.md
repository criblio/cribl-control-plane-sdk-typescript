...
async function run() {
  const result = await criblControlPlane.versions.branches.list();
...
import { versionsBranchesList } from "cribl-control-plane/funcs/versionsBranchesList.js";
...
async function run() {
  const res = await versionsBranchesList(criblControlPlane);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("versionsBranchesList failed:", res.error);
  }
}
...
**Promise\<[models.CountedBranchInfo](../../models/countedbranchinfo.md)\>**
...
async function run() {
  const result = await criblControlPlane.versions.branches.get();
...
```typescript
import { CriblControlPlaneCore } from "cribl-control-plane/core.js";
import { versionsBranchesGet } from "cribl-control-plane/funcs/versionsBranchesGet.js";
...
async function run() {
  const res = await versionsBranchesGet(criblControlPlane);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("versionsBranchesGet failed:", res.error);
  }
}
