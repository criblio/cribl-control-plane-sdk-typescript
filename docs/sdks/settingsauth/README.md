...
async function run() {
  const result = await criblControlPlane.system.settings.auth.list();
...
import { systemSettingsAuthList } from "cribl-control-plane/funcs/systemSettingsAuthList.js";
...
async function run() {
  const res = await systemSettingsAuthList(criblControlPlane);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("systemSettingsAuthList failed:", res.error);
  }
}
