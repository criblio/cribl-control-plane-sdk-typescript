...
* [getBranch](#getbranch) - get the list of branches
* [createCommit](#createcommit) - create a new commit containing the current configs the given log message describing the changes.
* [getFileCount](#getfilecount) - get the count of files of changed
* [getBranchName](#getbranchname) - returns git branch that the config is checked out to, if any
* [getDiff](#getdiff) - get the textual diff for given commit
* [getFileInfo](#getfileinfo) - get the files changed
* [getConfigStatus](#getconfigstatus) - Get info about versioning availability
* [pushCommit](#pushcommit) - push the current configs to the remote repository.
* [revertCommit](#revertcommit) - revert a commit
* [getDiffLogMessage](#getdifflogmessage) - get the log message and textual diff for given commit
* [getCurrentStatus](#getcurrentstatus) - get the the working tree status
* [syncLocalRemote](#synclocalremote) - syncs with remote repo via POST requests
* [restoreCommit](#restorecommit) - undo the last commit
...
async function run() {
  const result = await criblControlPlane.versioning.getDiffLogMessage({
    commit: "<value>",
    group: "<value>",
    filename: "example.file",
...
```typescript
import { CriblControlPlaneCore } from "cribl-control-plane/core.js";
import { versioningGetDiffLogMessage } from "cribl-control-plane/funcs/versioningGetDiffLogMessage.js";
...
async function run() {
  const res = await versioningGetDiffLogMessage(criblControlPlane, {
    commit: "<value>",
    group: "<value>",
    filename: "example.file",
    diffLineLimit: 7771.94,
  ...
  if (res.ok) {
    ...
    console.log("versioningGetDiffLogMessage failed:", res.error);
  }
}
...
async function run() {
  const result = await criblControlPlane.versioning.restoreCommit({
    group: "<value>",
  });
...
```typescript
import { CriblControlPlaneCore } from "cribl-control-plane/core.js";
import { versioningRestoreCommit } from "cribl-control-plane/funcs/versioningRestoreCommit.js";
...
async function run() {
  const res = await versioningRestoreCommit(criblControlPlane, {
    group: "<value>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("versioningRestoreCommit failed:", res.error);
  }
}
