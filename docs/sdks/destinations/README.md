...
* [list](#list) - Get a list of Destination objects
* [create](#create) - Create Destination
* [get](#get) - Get Destination by ID
* [update](#update) - Update Destination
* [delete](#delete) - Delete Destination
* [deletePersistentQueue](#deletepersistentqueue) - Clears destination persistent queue
* [getPersistentQueueStatus](#getpersistentqueuestatus) - Retrieves status of latest clear PQ job for a destination
* [getSampleData](#getsampledata) - Retrieve samples data for the specified destination. Used to get sample data for the test action.
* [createSampleData](#createsampledata) - Send sample data to a destination to validate configuration or test connectivity
...
async function run() {
  const result = await criblControlPlane.destinations.deletePersistentQueue({
    id: "<id>",
  });
...
```typescript
import { CriblControlPlaneCore } from "cribl-control-plane/core.js";
import { destinationsDeletePersistentQueue } from "cribl-control-plane/funcs/destinationsDeletePersistentQueue.js";
...
async function run() {
  const res = await destinationsDeletePersistentQueue(criblControlPlane, {
    id: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("destinationsDeletePersistentQueue failed:", res.error);
  }
}
