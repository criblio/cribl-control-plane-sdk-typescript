...
async function run() {
  const result = await criblControlPlane.deployments.getSummary({
    mode: "worker",
  });
...
import { deploymentsGetSummary } from "cribl-control-plane/funcs/deploymentsGetSummary.js";
...
async function run() {
  const res = await deploymentsGetSummary(criblControlPlane, {
    mode: "worker",
  });
  if (res.ok) {
    ...
    console.log("deploymentsGetSummary failed:", res.error);
  }
}
