...
async function run() {
  const result = await criblControlPlane.nodes.getSummary({
    mode: "worker",
  });
...
import { nodesGetSummary } from "cribl-control-plane/funcs/nodesGetSummary.js";
...
async function run() {
  const res = await nodesGetSummary(criblControlPlane, {
    mode: "worker",
  });
  if (res.ok) {
    ...
    console.log("nodesGetSummary failed:", res.error);
  }
}
