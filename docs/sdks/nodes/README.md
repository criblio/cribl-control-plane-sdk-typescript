...
* [getCount](#getcount) - get worker and edge nodes count
* [list](#list) - get worker and edge nodes
...
async function run() {
  const result = await criblControlPlane.nodes.getCount({
    filterExp: "<value>",
  });
...
import { nodesGetCount } from "cribl-control-plane/funcs/nodesGetCount.js";
...
async function run() {
  const res = await nodesGetCount(criblControlPlane, {
    filterExp: "<value>",
  });
  if (res.ok) {
    ...
    console.log("nodesGetCount failed:", res.error);
  }
}
