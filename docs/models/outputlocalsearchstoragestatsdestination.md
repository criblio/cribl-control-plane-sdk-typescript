# OutputLocalSearchStorageStatsDestination

## Example Usage

```typescript
import { OutputLocalSearchStorageStatsDestination } from "cribl-control-plane/models";

let value: OutputLocalSearchStorageStatsDestination = {};
```

## Fields

| Field                 | Type                  | Required              | Description           |
| --------------------- | --------------------- | --------------------- | --------------------- |
| `url`                 | *string*              | :heavy_minus_sign:    | N/A                   |
| `database`            | *string*              | :heavy_minus_sign:    | N/A                   |
| `tableName`           | *string*              | :heavy_minus_sign:    | N/A                   |
| `authType`            | *string*              | :heavy_minus_sign:    | N/A                   |
| `username`            | *string*              | :heavy_minus_sign:    | N/A                   |
| `sqlUsername`         | *string*              | :heavy_minus_sign:    | N/A                   |
| `password`            | *string*              | :heavy_minus_sign:    | N/A                   |
| `waitForAsyncInserts` | *boolean*             | :heavy_minus_sign:    | N/A                   |
| `concurrency`         | *number*              | :heavy_minus_sign:    | N/A                   |