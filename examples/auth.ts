...
export async function createCriblClient(): Promise<CriblControlPlane> {
  const criblAuth = isOnprem
    ? new AuthOnprem(configuration as OnpremConfiguration)
    : new AuthCloud(configuration as CloudConfiguration);
  return criblAuth.getClient();
}

/**
 * Validates and returns configuration based on deployment type.
 */
function getConfiguration(isOnprem: boolean): OnpremConfiguration | CloudConfiguration {
  if (isOnprem) {
