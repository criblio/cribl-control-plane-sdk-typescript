...
export type FunctionDnsLookupConf = {
  /**
...
/** @internal */
export const FunctionDnsLookupConf$inboundSchema: z.ZodType<
  FunctionDnsLookupConf,
  z.ZodTypeDef,
  unknown
> = collectExtraKeys$(
  z.object({
...
export function functionDnsLookupConfFromJSON(
  jsonString: string,
): SafeParseResult<FunctionDnsLookupConf, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => FunctionDnsLookupConf$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'FunctionDnsLookupConf' from JSON`,
  );
}
...
export const FunctionDnsLookup$inboundSchema: z.ZodType<
  ...
  unknown
> = z.object({
  __conf: z.lazy(() => FunctionDnsLookupConf$inboundSchema),
  __filename: z.string(),
  disabled: z.boolean(),
  group: z.string(),
