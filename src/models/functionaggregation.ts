...
export type FunctionAggregationConf = {
  /**
...
/** @internal */
export const FunctionAggregationConf$inboundSchema: z.ZodType<
...
export function functionAggregationConfFromJSON(
  jsonString: string,
): SafeParseResult<FunctionAggregationConf, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => FunctionAggregationConf$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'FunctionAggregationConf' from JSON`,
  );
}
...
export const FunctionAggregation$inboundSchema: z.ZodType<
  ...
  unknown
> = z.object({
  __conf: z.lazy(() => FunctionAggregationConf$inboundSchema),
  __filename: z.string(),
  disabled: z.boolean(),
  group: z.string(),
