/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v3";
import * as models from "../index.js";

export type GetProductsGroupsByProductAndIdRequest = {
  /**
   * Name of the Cribl product to get the Worker Groups, Outpost Groups, or Edge Fleets for.
   */
  product: models.ProductsCore;
  /**
   * The <code>id</code> of the Worker Group, Outpost Group, or Edge Fleet to get.
   */
  id: string;
  /**
   * Comma-separated list of additional properties to include in the response. Available values are <code>git.commit</code>, <code>git.localChanges</code>, and <code>git.log</code>.
   */
  fields?: string | undefined;
};

/** @internal */
export type GetProductsGroupsByProductAndIdRequest$Outbound = {
  product: string;
  id: string;
  fields?: string | undefined;
};

/** @internal */
export const GetProductsGroupsByProductAndIdRequest$outboundSchema: z.ZodType<
  GetProductsGroupsByProductAndIdRequest$Outbound,
  z.ZodTypeDef,
  GetProductsGroupsByProductAndIdRequest
> = z.object({
  product: models.ProductsCore$outboundSchema,
  id: z.string(),
  fields: z.string().optional(),
});

export function getProductsGroupsByProductAndIdRequestToJSON(
  getProductsGroupsByProductAndIdRequest:
    GetProductsGroupsByProductAndIdRequest,
): string {
  return JSON.stringify(
    GetProductsGroupsByProductAndIdRequest$outboundSchema.parse(
      getProductsGroupsByProductAndIdRequest,
    ),
  );
}
