...
import {
  CRIBL_CONTROL_PLANE_API_PREFIX,
  ensureControlPlaneApiV1Base,
  SDK_METADATA,
  SDKOptions,
...
export type RequestGroupNodeScope =
...
} & RequestGroupNodeScope & Omit<RequestInit, "method" | "body">;

function requestInitWithoutCriblSdkOptions(
...
export class ClientSDK {
  ...
  constructor(options: SDKOptions) {
    ...
    if (!this.#logger && env().CRIBLCONTROLPLANE_DEBUG) {
      this.#logger = console;
    }
  }
  ...
  public _createRequest(
    ...
    if (options?.group != null && options?.node != null) {
      return ERR(
        new InvalidRequestError(
          "Specify at most one of RequestOptions.group and RequestOptions.node",
        ),
      );
    }
    const baseURL = ensureControlPlaneApiV1Base(new URL(base));
    const marker = CRIBL_CONTROL_PLANE_API_PREFIX;
    const pathNoTrail = baseURL.pathname.replace(/\/+$/, "");
    const markerIdx = pathNoTrail.lastIndexOf(marker);
    if (options?.group != null) {
      if (markerIdx === -1) {
        return ERR(
          new InvalidRequestError(
            `Base URL must include ${marker} when using RequestOptions.group`,
          ),
        );
      }
      const prefix = pathNoTrail.slice(0, markerIdx + marker.length);
      baseURL.pathname =
        `${prefix}/m/${encodeURIComponent(options.group)}/`;
    } else if (options?.node != null) {
      if (markerIdx === -1) {
        return ERR(
          new InvalidRequestError(
            `Base URL must include ${marker} when using RequestOptions.node`,
          ),
        );
      }
      const prefix = pathNoTrail.slice(0, markerIdx + marker.length);
      baseURL.pathname =
        `${prefix}/w/${encodeURIComponent(options.node)}/`;
    }
    let reqURL: URL;
    if (path) {
    ...
    const fetchOptions: Omit<RequestInit, "method" | "body"> =
      requestInitWithoutCriblSdkOptions(options);
