...
export class ClientSDK {
  ...
  public _createRequest(
    ...
    const scope = this._options.requestScope;
    const group =
      scope && typeof scope === "object" && "group" in scope && scope.group != null
        ? scope.group
        : undefined;
    const node =
      scope && typeof scope === "object" && "node" in scope && scope.node != null
        ? scope.node
        : undefined;
    if (group != null && node != null) {
      return ERR(
        new InvalidRequestError("requestScope may not set both group and node"),
      );
    }
      ...
      if (markerIdx === -1) {
        return ERR(
          new InvalidRequestError(
            `Base URL must include ${marker} when using requestScope.group`,
          ),
        );
      }
      const prefix = pathNoTrail.slice(0, markerIdx + marker.length);
      baseURL.pathname = `${prefix}/m/${encodeURIComponent(group)}/`;
    } else if (node != null) {
      if (markerIdx === -1) {
        return ERR(
          new InvalidRequestError(
            `Base URL must include ${marker} when using requestScope.node`,
          ),
        );
      }
      const prefix = pathNoTrail.slice(0, markerIdx + marker.length);
      baseURL.pathname = `${prefix}/w/${encodeURIComponent(node)}/`;
    }
    let reqURL: URL;
    if (path) {
